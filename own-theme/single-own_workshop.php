		...
		<section id="workshop">
			<div class="wrapper">
				...
				<ul class="links">
					...
					<?php if ( get_field( 'email' ) ): ?><li><a href="mailto:<?php the_field( 'email_address' ) ?>"><?php the_field( 'email_address' ) ?></a></li><?php endif ?>
				</ul>
				...
				<?php if ( count( get_field( 'images' ) ) > 1 || get_field( 'images' )[0]['image'] ): ?>
					<ul class="photos">
						<?php foreach( get_field( 'images' ) as $i => $image ): ?>
							...
							<li class="photo">
								<div style="background-image: url( <?php echo wp_get_attachment_url( $image['image'] ) ?> );">
				...
				<?php 
					...
					$process_count = count( wp_get_post_terms( get_the_ID(), 'own_processes' ) );
					$tool_count = count( wp_get_post_terms( get_the_ID(), 'own_tools' ) );
				?>
				<?php if ( $service_count > 0 || $discipline_count > 0 || $material_count > 0 || $process_count > 0 || $tool_count > 0 ): ?>
					<section class="tax">
						...
						<?php if ( $process_count > 0 ): ?>
							<h2>Processes</h2>
							<p><?php OWN_Theme_Tax_List( 'own_processes' ) ?></p>
						<?php endif ?>
				...
				<?php if ( get_field( 'twitter' ) || get_field( 'facebook' ) || get_field( 'google+' ) || get_field( 'instagram' ) || get_field( 'google_group' ) || get_field( 'youtube' ) ): ?>
					<section class="links">
						<h2>Links</h2>
						<ul>
							<?php if ( get_field( 'twitter' ) ): ?><li><a href="http://twitter.com/<?php the_field( 'twitter' ) ?>" target="_blank">@<?php the_field( 'twitter' ) ?></a></li><?php endif ?>
							<?php if ( get_field( 'facebook' ) ): ?><li><a href="http://fb.com/<?php the_field( 'facebook' ) ?>" target="_blank">fb.com/<?php the_field( 'facebook' ) ?></a></li><?php endif ?>
							<?php if ( get_field( 'google+' ) ): ?><li><a href="http://google.com/+<?php the_field( 'google+' ) ?>" target="_blank">google.com/+<?php the_field( 'google+' ) ?></a></li><?php endif ?>
							<?php if ( get_field( 'instagram' ) ): ?><li><a href="http://instagram.com/<?php the_field( 'instagram' ) ?>" target="_blank">instagram.com/<?php the_field( 'instagram' ) ?></a></li><?php endif ?>
							<?php if ( get_field( 'google_group' ) ): ?><li><a href="http://groups.google.com/d/forum/<?php the_field( 'google_group' ) ?>" target="_blank">groups.google.com/d/forum/<?php the_field( 'google_group' ) ?></a></li><?php endif ?>
							<?php if ( get_field( 'youtube' ) ): ?><li><a href="http://youtube.com/<?php the_field( 'youtube' ) ?>" target="_blank">youtube.com/channel/<?php the_field( 'youtube' ) ?></a></li><?php endif ?>
						</ul>
					</section>
				<?php endif ?>
